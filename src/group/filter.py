...
from src.group.bingo import handle_bingo
from src.settings.config import ADMINS, LOG_GROUP
    ...
    for member in update.message.new_chat_members:
        if check_cas(member.id):
            ...
            context.bot.send_message(LOG_GROUP,
                                     f"User {mention_html(member.id, member.name)} [<code>{member.id}</code>] was banned in group "
                                     "<a href='https://t.me/username'>{update.message.group.title}</a> due"
